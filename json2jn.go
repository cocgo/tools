...
// qq:521401
// email: 521401@qq.com
// 功能：cocostudio导出的ui文件，可以转换为json.jn的文件，把微软雅黑字体换掉
...
func main() {
	...
	fmt.Println("zhangliangzhi，2015-11-03 14:01:50  v4.0 email: 521401@qq.com")
}
...
func JsonChange(fileName string) {
	...
	if strings.Contains(fileName, ".json.jn") {
		return
	}
...
func LineString(line string) string {
	...
	// 把 D:\\ProgramFiles(x86)\\CocosStudio\\EditorDefaultRes\\GUI/image.png -> 改为 image.png
	line = strings.Replace(line, "微软雅黑", "fonts/d.ttf", -1)
	if !strings.Contains(line, "GUI") {
		return line
	}
